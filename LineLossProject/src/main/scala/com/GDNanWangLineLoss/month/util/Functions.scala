...
object Functions {
    ...
    /** 3
    * @Description: 获取编码
    * @Param: [sparkSession, zz]
    * @return: org.apache.spark.sql.expressions.UserDefinedFunction
    * @Date: 2020/10/27
    */
    def getbm(sparkSession:SparkSession,zz:DataFrame)={
        var zzlxdmBroadcast: Broadcast[mutable.Map[String, String]] =null
        ...
        zzlxdmBroadcast= sparkSession.sparkContext.broadcast(zzlxdmMap)
        ...
        /*------获取区县局编码---------------------------------------------------------------------------------------*/
        sparkSession.udf.register("getQxjbm", (gddwbm: String) => { // 2020/10/27 调用这个函数名getQxjbm，传入这个值gddwbm
            gddwbm match {
                case null => "-"
                case _ =>{
                    val zzlxdm: String = zzlxdmBroadcast.value.get(gddwbm).getOrElse(null)
    ...
    def getDMbm(sparkSession:SparkSession)={
        ...
        sparkSession.udf.register("getdmbmmc", (dmfl: String, dmbm: String) => {// 2020/10/29   dmfl(string) comment '（冗余字段），方便数据查询', dmbm  (string) comment '代码分类的编码内容，如紧急程度代码中：代码编码->代码编码名称  1->特急 2->紧急 3-> 一般',
    ...
    /** 5
      ...
      * @return
      */
    def getBDZxx(sparkSession:SparkSession)={
