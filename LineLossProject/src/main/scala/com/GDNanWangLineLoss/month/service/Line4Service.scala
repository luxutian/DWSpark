        ...
        try{
            val xl_41 =
                s"""
                   ...
                   |    getbdzbs(d.xlxdbs) bdzbs,getbdzbh(d.xlxdbs) bdzbh,
                   |    getbdzmc(d.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,null tqkhbhh,
                   |    d.yhbh,d.yhmc,d.yhlbdm,
                   |    handleNumber(d.glys) glys,
                   |    handleNumber(d.jfdl) jfdl,
                   |    getycgzbh(${Constant.XL_41}) ycgzbh,getDsjbm(d.gddwbm) dsjbm,
                   ...
                   |    getdmbmmc('YHLBDM',d.yhlbdm) yhlb,getdqbm(d.gddwbm) dqbm,
                   |    ${nybm} nydm
                   |from (
                   |    select t.xlxdbs
                   |    from jldxx t
                   ...
                   |            from jldxx t join xlxd l on t.xlxdbs = l.xlxdbs
                   |            where t.dfny = ${nowMonth} and t.yhlbdm = '60'
        ...
        try{
            val xl_45 =
                s"""
                   ...
                   |    null tqbs,null tqbh,null tqmc,
                   |    getbdzbs(xs.xlxdbs) bdzbs,getbdzbh(xs.xlxdbs) bdzbh,getbdzmc(xs.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,null tqkhbhh,
                   |    y.sbbs byqbh,y.mc byqmc,y.gisid byqgisid,y.sbxhdm,y.yxztdm,
                   ...
                   |    getdmbmmc('BYQYXZTDM',y.yxztdm) yhzt,getdqbm(xs.gddwbm) dqbm,
                   |    ${nybm} nybm
                   |from xlxstjxx xs
                   |inner join xltqgx x on x.xlxdbs = xs.xlxdbs  --线路台区关系
                   |inner join yxbyq y on y.tqbs = x.tqbs and y.shlxdm in ('01','02','03') and y.gbzbbz = '1' --运行变压器
                   |inner join (
                   |            select j.xlxdbs,sum(j.jfdl) gbzdl
                   |            from jldxx j
                   |            inner join yxbyq b on b.tqbs = j.tqbs and b.shlxdm in ('01','02','03') and b.gbzbbz = '1'
            ...
            //高损公用变压器清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_gsgybyqqd")
...
//        sparkSession.sql(s"select * from ruleState union all select getFormatDate(${end}) recordtime,'xl45',${isSuccess} state,'${reason}' reason,${(end-start)/1000} runtime").createOrReplaceTempView("ruleState")
        println(s"规则4.2运行${(end-start)/1000}秒")
        ...
        try{
            val xl_46 =
                s"""
                   ...
                   |    k1.bdzkhbhh,null tqkhbhh,
                   |    m.code fhqggzdh,cast(m.apply_time as string) qdsj,
                   |    getycgzbh(${Constant.XL_46}) ycgzbh,
                   ...
                   |    getzzmc(getGdsbm(l.gddwbm)) gds,getdqbm(l.gddwbm) dqbm,
                   |    ${nybm} nybm
                   |from xlxd l
            ...
            //负荷切割工作单清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_fhqggzdqd")
...
//        sparkSession.sql(s"select * from ruleState union all select getFormatDate(${end}) recordtime,'xl46',${isSuccess} state,'${reason}' reason,${(end-start)/1000} runtime").createOrReplaceTempView("ruleState")
        ...
        try{
            val xl_47 =
                s"""
                   ...
                   |    ${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,l.xlxdbs,l.xlbh,l.xlmc,
                   |    null tqbs,null tqbh,null tqmc,
                   |    l.bdzbs,l.bdzbh,l.bdzmc,
                   |    k1.bdzkhbhh,null tqkhbhh,
                   |    y.sbbs,y.mc,cast(b.zxygbs as string)b.zxygbs shdl,
                   ...
                   |    ${nybm} nybm
                   |from xlxstjxx l
                   |inner join xltqgx x on x.xlxdbs = l.xlxdbs
                   |inner join yxbyq y on y.tqbs = x.tqbs and y.gbzbbz = '1' and y.yxztdm = '01'
                   |inner join byqygdsh b on b.sbbs = y.sbbs  --变压器月固定损耗
                   |inner join (
                   |            select
                   ...
                   |            inner join xltqgx x on x.xlxdbs = l.xlxdbs
                   |            inner join yxbyq y on y.tqbs = x.tqbs and y.gbzbbz = '1' and y.yxztdm = '01'
                   |            inner join byqygdsh b on b.sbbs = y.sbbs
                   |            where l.ny = ${nowMonth}
                   |            group by l.xlxdbs
                   |            ) a
            ...
            //公变变损清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_gbbsqd").show(5)
            sparkSession.sql(s"select getUUID() id,* from res_gk_gbbsqd")
...
//        sparkSession.sql(s"select * from ruleState union all select getFormatDate(${end}) recordtime,'xl47',${isSuccess} state,'${reason}' reason,${(end-start)/1000} runtime").createOrReplaceTempView("ruleState")
        ...
        //线路4.5 同组合的线路阈值信息     这里的变压器不用join，直接从_xlxstjxx可以拿到
        start = System.currentTimeMillis()
        try{
            val xl_49 =
                s"""
                   ...
                   |    distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,
                   |    x.gddwbm,
                   |    ${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,b.xlxdbs,b.xlbh,b.xlmc,
                   |    null tqbs,null tqbh,null tqmc,
                   |    x.bdzbs,x.bdzbh,x.bdzmc,
                   |    k1.bdzkhbhh,null tqkhbhh,
                   |    b.xlbzbh,
                   |    handleNumber(x.byxsl) zhhxsl,
                   |    handleNumber(x.khzb) xszb,
                   |    getycgzbh(${Constant.XL_49}) ycgzbh,getDsjbm(x.gddwbm) dsjbm,
                   |    getQxjbm(x.gddwbm) qxjbm,getGdsbm(x.gddwbm) gdsbm,
                   |    getzzmc(getDsjbm(x.gddwbm)) dsj,getzzmc(getQxjbm(x.gddwbm)) qxj,
                   |    getzzmc(getGdsbm(x.gddwbm)) gds,getdqbm(x.gddwbm) dqbm,
                   |    ${nybm} nybm
            ...
            //组合线路阈值清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_zhxlfzqd").show(5)
            sparkSession.sql(s"select getUUID() id,* from res_gk_zhxlfzqd")
        ...
        try{
            val xl_410 =
                s"""
                   ...
                   |    cast(d.execute_begin_time as string) hwkssj,
                   |    cast(d.execute_end_time as string) hwjssj,
                   |    null zcglxthwsj,m.code zcglxtgdbh,null zcglxtgdmc,
                   |    getycgzbh(${Constant.XL_410}) ycgzbh,
                   |    getDsjbm(l.gddwbm) dsjbm,getQxjbm(l.gddwbm) qxjbm,getGdsbm(l.gddwbm) gdsbm,
                   |    getzzmc(getDsjbm(l.gddwbm)) dsj,getzzmc(getQxjbm(l.gddwbm)) qxj,
                   |    getzzmc(getGdsbm(l.gddwbm)) gds,getdqbm(l.gddwbm) dqbm,
                   |    ${nybm} nybm
            ...
            //实际环网未组合线路清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_sjhwwzhxlqd").show(5)
            sparkSession.sql(s"select getUUID() id,* from res_gk_sjhwwzhxlqd")
