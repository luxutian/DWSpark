        ...
        try{
            val tq_21 =
                s"""
                   ...
                   |    j.gddwbm,
                   |    ${nowMonth} tjsj,${ybs} tjzq,${tqbz} xltqbz,xl.xlxdbs,xl.xlbh,
                   |    xl.xlmc,xl.tqbs,xl.tqbh,
                   |    xl.tqmc,getbdzbs(xl.xlxdbs) bdzbs,getbdzbh(xl.xlxdbs) bdzbh,getbdzmc(xl.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    j.yhbh,j.yhmc,j.yhlbdm,jl.gddwbm sygddwbm,
                   ...
                   |    getdmbmmc('YHLBDM',j.yhlbdm) yhlb,getdqbm(xl.gddwbm) dqbm,
                   |    ${nybm} nybm
            ...
            //初始化供电单位变更清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_cshgddwbgqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_cshgddwbgqd")
...
//        sparkSession.sql(s"select * from ruleState union all select getFormatDate(${end}) recordtime,'tq21',${isSuccess} state,'${reason}' reason,${(end-start)/1000} runtime").createOrReplaceTempView("ruleState")
        ...
        try{
            ...
            //供电单位为空户变更清单
            sparkSession.sql(s"select getUUID(),* from res_gk_gddwwkhbgqd").show(5)
            sparkSession.sql(s"select getUUID(),* from res_gk_gddwwkhbgqd")
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_gddwwkhbgqd")
        ...
        try{
            val tq_23 =
                s"""
                   ...
                   |    distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,
                   |    xl.gddwbm,
                   |    ${nowMonth} tjsj,${ybs} tjzq,${tqbz} xltqbz,xl.xlxdbs,
                   |    xl.xlbh,xl.xlmc,xl.tqbs,
                   |    xl.tqbh,xl.tqmc,getbdzbs(xl.xlxdbs) bdzbs,getbdzbh(xl.xlxdbs) bdzbh,getbdzmc(xl.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    j.yhbh,j.yhmc,j.yhlbdm,jl.jldydjdm sqdydjdm,
                   ...
                   |    getdmbmmc('DYDJDM',j.jldydjdm) DYDJ,getdqbm(xl.gddwbm) dqbm,
                   |    ${nybm} nybm
                   |from xsycxlhtqmx xl  --线损异常(线路和台区)明细
                   |inner join jldxx j on j.xlxdbs = xl.xlxdbs and j.jldytdm <> '410' and j.dfny = ${nowMonth}
                   |inner join jldxx jl on jl.yhbh = j.yhbh and jl.jldytdm <> '410' and  jl.dfny = ${lastMonth}
            ...
            sparkSession.sql("select * from res_gk_dfdchdydjbgqd where isFiveDsj(gddwbm) = 1")
              .repartition(resultPartition).createOrReplaceTempView("res_gk_dfdchdydjbgqd")

            //地方电厂户电压等级变更清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_dfdchdydjbgqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_dfdchdydjbgqd")
...
//        sparkSession.sql(s"select * from ruleState union all select getFormatDate(${end}) recordtime,'tq23',${isSuccess} state,'${reason}' reason,${(end-start)/1000} runtime").createOrReplaceTempView("ruleState")
        ...
        try{
            ...
            //光伏用户类别有误清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_gfyhlbywqd").show(5)
            sparkSession.sql(s"select getUUID() id,* from res_gk_gfyhlbywqd")
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_gfyhlbywqd")
        ...
        try{
            val tq_25 =
                s"""
                   ...
                   |    distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,
                   |    tj.gddwbm,
                   |    ${nowMonth} tjsj,${ybs} tjzq,${tqbz} xltqbz,tj.xlxdbs,tj.xlbh,tj.xlmc,
                   |    tj.tqbs,tj.tqbh,tj.tqmc,
                   |    bd.bdzbs,bd.bdzbh,bd.bdzmc,
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    y.cbzq,
                   |    handleNumber(tj.bygdl) gdl,
                   |    handleNumber(tj.bysdl) sdl,
                   |    getycgzbh(${Constant.TQ_25}) ycgzbh,getDsjbm(tj.gddwbm) dsjbm,
                   ...
                   |    getdmbmmc('CBZQDM',y.cbzq) cbzq,getdqbm(tj.gddwbm) dqbm,
                   |    ${nybm} nybm
                   |from ydkh y
                   |inner join jld j on j.yhbh = y.yhbh and j.jldytdm <> '410'
            ...
            //抄表周期与电量不匹配用户清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_cbzqydlbppyhqd").show(5)
            sparkSession.sql(s"select getUUID() id,* from res_gk_cbzqydlbppyhqd")
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_cbzqydlbppyhqd")
        ...
        try{
            val tq_26 =
                s"""
                   ...
                   |    k1.bdzkhbhh,null tqkhbhh,y.yhbh,y.yhmc,y.yhlbdm,
                   |    getycgzbh(${Constant.TQ_26}) ycgzbh,
                   ...
                   |    getzzmc(getGdsbm(j.gddwbm)) gds,getdmbmmc('YHLBDM',y.yhlbdm) yhlb,getdqbm(j.gddwbm) dqbm,
                   |    ${nybm} nybm
                   |from ydkh y
            ...
            //计量点信息档案所属线路为空清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_jldxxdassxlwkqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_jldxxdassxlwkqd")
        ...
        try{
            val tq_27 =
                s"""
                   ...
                   |    xstq.tqbs,xstq.tqbh,xstq.tqmc,
                   |    getbdzbs(xsxl.xlxdbs) bdzbs,
                   ...
                   |    khxl.xlmc khzhdaxlmc,j.tqbs khzhdatqbs,
                   ...
                   |    getdmbmmc('YHLBDM',y.yhlbdm) yhlb,getdqbm(xstq.gddwbm) dqbm,
                   |    ${nybm} nybm
                   |from ydkh y
                   ...
                   |    distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,
                   |    xstq.gddwbm,
                   |    ${nowMonth} tjsj,${ybs} tjzq,${tqbz} xltqbz,
                   |    xsxl.xlxdbs,xsxl.xlbh,xsxl.xlmc,
                   ...
                   |    khtq.tqbh khzhdatqbh,khtq.tqmc khzhdatqmc,
                   |    getycgzbh(${Constant.TQ_27}) ycgzbh,
            ...
            //计量点档案与用户档案不一致清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_yxljlddayyhdabyzqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_yxljlddayyhdabyzqd")
        ...
        try{
            val tq_28 =
                s"""
                   ...
                   |    distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,
                   |    t.gddwbm,
                   |    ${nowMonth} tjsj,${ybs} tjzq,${tqbz} xltqbz,l.xlxdbs,
                   |    l.xlbh,l.xlmc,
                   |    t.tqbs,t.tqbh,t.tqmc,
                   |    getbdzbs(l.xlxdbs) bdzbs,getbdzbh(l.xlxdbs) bdzbh,getbdzmc(l.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    y.yhbh,y.yhmc,y.yhlbdm,b.sbbs byqbs,b.mc byqmc,
                   |    b.sbbs byqbh,b.gisid byqgisid,
                   |    null yxxtdydxlxdbs,null yxxtdydxlbh,null yxxtdydxlmc,
                   |    null yxxtdydxlgisid,null gisxsdydxlxdbs,null gisxsdydtqgisidgisxsdydtqgisid,
                   |    null gisxsdydxlmc,null gisxsdydxlgisid,
                   ...
                   |    g.trans_id gisxsdydtqgisid,
                   |    getycgzbh(${Constant.TQ_28}) ycgzbh,
                   |    getDsjbm(t.gddwbm) dsjbm,getQxjbm(t.gddwbm) qxjbm,getGdsbm(t.gddwbm) gdsbm,
            ...
            //GIS系统和营销系统电源点不一致清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_gisxthyxxtdydbyzqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_gisxthyxxtdydbyzqd")
        ...
        try{
            val tq_29 =
                s"""
                   ...
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    y.yhbh,y.yhmc,y.yhlbdm,y.cbqdbh,cb.cbqdmc,
            ...
            //抄表区段不一致用户清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_cbqdbyzyhqd").show(5)
            sparkSession.sql(s"select getUUID() id,* from res_gk_cbqdbyzyhqd")
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_cbqdbyzyhqd")
        ...
        try{
            val tq_210 =
                s"""
                   ...
                   |    distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,
                   |    t.gddwbm,
                   |    ${nowMonth} tjsj,${ybs} tjzq,${tqbz} xltqbz,t.xlxdbs,t.xlbh,t.xlmc,
                   |    t.tqbs,t.tqbh,t.tqmc,
                   |    getbdzbs(t.xlxdbs) bdzbs,getbdzbh(t.xlxdbs) bdzbh,getbdzmc(t.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    y.yhbh,y.yhmc,y.yhlbdm,a.tqbs dydxxtqbs,a.tqbh dydxxtqbh,
                   |    a.tqmc dydxxtqmc,a.yhbh dydxxtqkhhhh,b.tqbs jzqzdgxtqbs,b.tqbh jzqzdgxtqbh,
                   |    b.tqmc jzqzdgxtqmc,b.yhbh jzqzdgxtqkhhhh,
                   |    getycgzbh(${Constant.TQ_210}) ycgzbh,
                   |    getDsjbm(t.gddwbm) dsjbm,getQxjbm(t.gddwbm) qxjbm,getGdsbm(t.gddwbm) gdsbm,
                   |    getzzmc(getDsjbm(t.gddwbm)) dsj,getzzmc(getQxjbm(t.gddwbm)) qxj,
                   |    getzzmc(getGdsbm(t.gddwbm)) gds,getdmbmmc('YHLBDM',y.yhlbdm) yhlb,getdqbm(t.gddwbm) dqbm,
                   |    ${nybm} nybm
            ...
            //所属台区与集中器终端关系不一致用户清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_sstqyjzqzdgxbyzyhqd").show(5)
            sparkSession.sql(s"select getUUID() id,* from res_gk_sstqyjzqzdgxbyzyhqd")
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_sstqyjzqzdgxbyzyhqd")
...
//        sparkSession.sql(s"select * from ruleState union all select getFormatDate(${end}) recordtime,'tq210',${isSuccess} state,'${reason}' reason,${(end-start)/1000} runtime").createOrReplaceTempView("ruleState")
        ...
        try{
            ...
            //考核表反向有功电量计量点缺失清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_khbfxygdkjkdqsqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_khbfxygdkjkdqsqd")
        ...
        try{
            val tq_212 =
                s"""
                   ...
                   |    getbdzbs(l.xlxdbs) bdzbs,getbdzbh(l.xlxdbs) bdzbh,getbdzmc(l.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    y.yhbh,y.yhmc,y.yhlbdm,y.yhztdm yxztdm,
                   |    handleNumber(x.jfdl) jfdl,
                   ...
                   |    getdmbmmc('YXZT',y.yhztdm) yhzt,getdqbm(t.gddwbm) dqbm,
                   |    ${nybm} nybm
                   |from ydkh y
            ...
            //运行状态与电量不匹配清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_yxztydlbppqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_yxztydlbppqd")
        ...
        try{
            val tq_213 =
                s"""
                   ...
                   |    getbdzmc(l.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    y.yhbh,y.yhmc,y.yhlbdm,j.jfdl ydl,
                   |    d.ykgdbh gdbh,
                   |    handleTime(g.sqsj) qdsj,
                   |    handleTime(g.wcsj) gdsj,
                   |    getycgzbh(${Constant.TQ_213}) ycgzbh,
                   |    getDsjbm(t.gddwbm) dsjbm,getQxjbm(t.gddwbm) qxjbm,getGdsbm(t.gddwbm) gdsbm,
                   |    getzzmc(getDsjbm(t.gddwbm)) dsj,getzzmc(getQxjbm(t.gddwbm)) qxj,
                   |    getzzmc(getGdsbm(t.gddwbm)) gds,getdmbmmc('YHLBDM',y.yhlbdm) yhlb,getdqbm(t.gddwbm) dqbm,
                   |    ${nybm} nybm
            ...
            //档案变更电子化移交不及时清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_dabgdzhyjbjsqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_dabgdzhyjbjsqd")
        ...
        try{
            val tq_214 =
                s"""
                   ...
                   |    getdmbmmc('JLFSDM',j.jlfsdm) jlfs,
            ...
            //计量辅助判断用户类别异常清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_jlfzpdyhlbycqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_jlfzpdyhlbycqd")
        ...
        try{
            val tq_215 =
                s"""
                   ...
                   |    distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,
                   |    xl.gddwbm,
                   |    ${nowMonth} tjsj,${ybs} tjzq,${tqbz} xltqbz,xl.xlxdbs,xl.xlbh,xl.xlmc,
                   |    xl.tqbs,xl.tqbh,xl.tqmc,
                   |    getbdzbs(xl.xlxdbs) bdzbs,getbdzbh(xl.xlxdbs) bdzbh,getbdzmc(xl.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,k2.tqkhbhh,
                   |    y.yhbh,y.yhmc,y.yhlbdm,yw.ywbm ywlx,g.gzdbh ywgzdbh,
                   |    handleNumber(j.jfdl) jfdl,
                   |    getycgzbh(${Constant.TQ_215}) ycgzbh,getDsjbm(xl.gddwbm) dsjbm,
                   |    getQxjbm(xl.gddwbm) qxjbm,getGdsbm(xl.gddwbm) gdsbm,getzzmc(getDsjbm(xl.gddwbm)) dsj,
                   |    getzzmc(getQxjbm(xl.gddwbm)) qxj,getzzmc(getGdsbm(xl.gddwbm)) gds,
                   |    getdmbmmc('YHLBDM',y.yhlbdm) yhlb,yw.ywbmmc ywlx,getdqbm(xl.gddwbm) dqbm,
                   |    ${nybm} nybm
                   |from xsycxlhtqmx xl
            ...
            //计量、业扩工单翻月清单
            sparkSession.sql(s"select getUUID() id,* from res_gk_jlykgdfyqd").show(5)
...
//            sparkSession.sql(s"select distinct ${creator_id} creator_id,${create_time} create_time,${update_time} update_time,${updator_id} updator_id,gddwbm,xltqbz,xlxdbs,tqbs,tjzq,tjsj,ycgzbh,getycgzmc(ycgzbh) from res_gk_jlykgdfyqd")
