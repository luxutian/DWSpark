...
object Line1Service {
    ...
    /**
      ...
      * @param url
      */
    def line1Calculate(sparkSession: SparkSession, url:String)={
        ...
        try{
            ...
            val xl_11 =
                s"""
                   ...
                   |    l.gddwbm,${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,l.xlxdbs,l.xlbh,
                   |    l.xlmc,null,null,null,getbdzbs(l.xlxdbs) bdzbs,getbdzbh(l.xlxdbs) bdzbh,
                   |    getbdzmc(l.xlxdbs) bdzmc,k1.bdzkhbhh,null,y.yhbh,y.yhmc,y.yhlbdm yhlbdm,
                   |    j.jldbh,c.zcbh,handleNumber(c.zhbl),getycgzbh(${Constant.XL_11}) ycgzbh,
                   ...
                   |    getdqbm(l.gddwbm) dqbm,${nybm}
                   |from cbxx c
                   |join ydkh y on c.yhbh=y.yhbh
                   |join jld j on j.jldbh=c.jldbh and j.jldytdm <> '410' --这个410 代表什么含义
                   |join xlxd l on l.xlxdbs=j.xlxdbs
                   |join xlcjwzl w on w.xlxdbs = l.xlxdbs and w.cjwzl < 100
                   ...
                   |    and ((c.yhlbdm in ('80','60') and c.sslxdm in ('121','221')) or (c.yhlbdm in ('10','11') and c.sslxdm = '121')
                   |    or (c.yhlbdm = '40' and c.sslxdm in ('221','222','223','224')))
                   |    and (y.yhlbdm in ('80','60','10','11') or (y.yhlbdm = '40' and exists (select 1 from _dcfzxx b where b.dcbh = y.yhbh and b.dydj = '08')))
                   |    and y.yhztdm <> '2
                   |
                 """.stripMargin

            sparkSession.sql(xl_11).createOrReplaceTempView("RES_GK_WCJYHQD")
            sparkSession.sql("select * from RES_GK_WCJYHQD where isFiveDsj(gddwbm) = 1")  //todo 这里点where 就可以省略优化上一行
              .repartition(resultPartition).createOrReplaceTempView("res_gk_wcjyhqd")

            //月线路未采集用户清单(表)
            sparkSession.sql(s"select getUUID(),*,tjsj fqrq from res_gk_wcjyhqd")  //tjsj 年月
              .write.options(Map("kudu.master" -> "url","kudu.table" -> s" ${writeSchema}.gk_yxlwcjyhqd"))
        ...
        //todo 2 线路1.2 起止表码时标平移检查   fzqjfdl未确定
        ...
        try{
            val xl_12 =
                s"""
                   ...
                   |    null,null,null,getbdzbs(l.xlxdbs) bdzbs,getbdzbh(l.xlxdbs) bdzbh,
                   |    getbdzmc(l.xlxdbs) bdzmc,k1.bdzkhbhh,null,c.yhbh,c.yhmc,y.yhlbdm,c.jldbh,
                   |    c.zcbh,handleNumber(c.scbss) qm,handleTime(c.sccbrq) qmcjsj,handleNumber(c.bcbss) zm,
                   |	handleTime(c.cbsj) zmcjsj,handleNumber(c.zhbl),handleNumber(c.bjdl) dl,
                   |    null fzqjfdl,getycgzbh(${Constant.XL_12}) ycgzbh,getDsjbm(l.gddwbm) dsjbm,
                   |	getQxjbm(l.gddwbm) qxjbm,getGdsbm(l.gddwbm) gdsbm,
                   |    getzzmc(getDsjbm(l.gddwbm)) dsj,getzzmc(getQxjbm(l.gddwbm)) qxj,
                   |	getzzmc(getGdsbm(l.gddwbm)) gds,
                   |    getdmbmmc('YHLBDM',y.yhlbdm) yhlb,getdqbm(l.gddwbm) dqbm,${nybm}
                   |from cbxx c
        ...
        try{
            val xl_13 =
                s"""
                   ...
                   |    l.xlxdbs,l.xlbh,l.xlmc,null,null,null,getbdzbs(l.xlxdbs) bdzbs,
                   |    getbdzbh(l.xlxdbs) bdzbh,getbdzmc(l.xlxdbs) bdzmc,k1.bdzkhbhh,null,
                   |    c.yhbh,c.yhmc,y.yhlbdm,c.jldcbfsdm cbfsdm,c.jldbh,
                   |    c.zcbh,handleNumber(c.scbss) qm,handleTime(c.sccbrq) qmcjsj,
                   |    handleNumber(c.bcbss) zm,handleTime(c.cbsj) zmcjsj,
                   |    handleNumber(c.zhbl),handleNumber(c.bjdl) dl,
                   |    getycgzbh(${Constant.XL_13}) ycgzbh,getDsjbm(l.gddwbm) dsjbm,
                   |    getQxjbm(l.gddwbm) qxjbm,getGdsbm(l.gddwbm) gdsbm,
                   |    getzzmc(getDsjbm(l.gddwbm)) dsj,getzzmc(getQxjbm(l.gddwbm)) qxj,
                   |    getzzmc(getGdsbm(l.gddwbm)) gds,getdmbmmc('YHLBDM',y.yhlbdm) yhlb,
                   |    getdmbmmc('CBFSDM',c.jldcbfsdm) cbfs,getdqbm(l.gddwbm) dqbm,${nybm},
        ...
        try{
            val xl_14 =
                s"""
                   ...
                   |    distinct ${creator_id},${create_time},${update_time},${updator_id},
                   |    l.gddwbm,${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,
                   |    l.xlxdbs,l.xlbh,l.xlmc,null,null,null,getbdzbs(j.xlxdbs) bdzbs,
                   |    getbdzbh(j.xlxdbs) bdzbh,getbdzmc(j.xlxdbs) bdzmc,
                   |    k1.bdzkhbhh,null,y.yhbh,y.yhmc,y.yhlbdm,c.jldbh,c.zcbh,
                   ...
                   |join xlxd l on l.xlxdbs = j.xlxdbs
                   |join jlbmhb b on b.yhbh = c.yhbh and b.jldbh = c.jldbh and b.bjzcbh = c.zcbh  --jlbmhb 这个表找不到
                   |lateral view outer explode(split(getbdzkhb(l.xlxdbs),',')) k1 as bdzkhbhh
        ...
        try{
            ...
            val xl_15 =
                s"""
                   ...
                   |    distinct ${creator_id} ,${create_time},${update_time},${updator_id},
                   |    l.gddwbm,${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,
                   |    j1.xlxdbs,l.xlbh,l.xlmc,null,null,null,getbdzbs(j1.xlxdbs) bdzbs,
                   |    getbdzbh(j1.xlxdbs) bdzbh,getbdzmc(j1.xlxdbs) bdzmc,k1.bdzkhbhh,null,j
                   |    1.yhbh,j1.yhmc,j1.yhlbdm,j1.jldbh,c1.zcbh,handleNumber(c1.scbss) bydlqm,
                   ...
                   |    handleNumber(j2.jfdl),x.xgxs,getycgzbh(${Constant.XL_15}) ycgzbh,
                   ...
                   |    getdmbmmc('YHLBDM',j1.yhlbdm) yhlb,getdqbm(l.gddwbm) dqbm,${nybm}
                   |from xgxs x
                   ...
                   |join xlxd l on l.xlxdbs = j1.xlxdbs
...
//            sparkSession.sql(s"insert into ${writeSchema}.gk_dlycbdyhqd_his select getUUID(),*,tjsj fqrq from res_gk_dlycbdyhqd")
        ...
        try{
            ...
            val xl_16 =
                s"""
                   |select distinct ${creator_id},${create_time},${update_time},${updator_id},l.gddwbm,${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,
                   |      l.xlxdbs,l.xlbh,l.xlmc,null,null,null,getbdzbs(l.xlxdbs) bdzbs,getbdzbh(l.xlxdbs) bdzbh,getbdzmc(l.xlxdbs) bdzmc,
                   |      k1.bdzkhbhh,null,a.yhbh,a.yhmc,a.yhlbdm,cb.jldbh,cb.zcbh,handleNumber(b.dl5),handleNumber(b.dl4),handleNumber(b.dl3),
                   |      handleNumber(b.dl2),handleNumber(b.dl1),handleNumber(a.yhjfdl),handleNumber(c.yhpjjfdl),
                   |      handleNumber(round((a.yhjfdl-c.yhpjjfdl)/c.yhpjjfdl*100,6)) hbbdl,handleNumber(round(d.tbbdl*100,6)),
                   |      handleNumber(round(a.yhjfdl/l.bygdl*100,6)) dlzxsgdlbl,getycgzbh(${Constant.XL_16}) ycgzbh,
        ...
        try{
            val xl_17 =
                s"""
                   ...
                   |      handleNumber(c.scbss) qm,handleNumber(c.bcbss) zm,
                   |      handleNumber(c.zhbl),handleNumber(c.bjdl) dl,d.gzdbh tbgdh,
                   |      handleNumber(d.ygdl) tbdl,getycgzbh(${Constant.XL_17}) ycgzbh,
                   |      getDsjbm(l.gddwbm) dsjbm,getQxjbm(l.gddwbm) qxjbm,getGdsbm(l.gddwbm) gdsbm,
                   |      getzzmc(getDsjbm(l.gddwbm)) dsj,
                   |      getzzmc(getQxjbm(l.gddwbm)) qxj,getzzmc(getGdsbm(l.gddwbm)) gds,
                   |      getdmbmmc('YHLBDM',j.yhlbdm) yhlb,getdqbm(l.gddwbm) dqbm,${nybm}
                   |from jldxx j
                   ...
                   |select distinct ${creator_id},${create_time},${update_time},${updator_id},l.gddwbm,${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,
                   |      l.xlxdbs,l.xlbh,l.xlmc,null,null,null,getbdzbs(l.xlxdbs) bdzbs,getbdzbh(l.xlxdbs) bdzbh,getbdzmc(l.xlxdbs) bdzmc,
                   |      k1.bdzkhbhh,null,j.yhbh,j.yhmc,j.yhlbdm,j.jldbh,c.zcbh,handleNumber(c.scbss) qm,handleNumber(c.bcbss) zm,handleNumber(c.zhbl),
                   |      handleNumber(c.bjdl) dl,d.gzdbh tbgdh,handleNumber(d.tbdl) tbdl,getycgzbh(${Constant.XL_17}) ycgzbh,getDsjbm(l.gddwbm) dsjbm,getQxjbm(l.gddwbm) qxjbm,getGdsbm(l.gddwbm) gdsbm,getzzmc(getDsjbm(l.gddwbm)) dsj,getzzmc(getQxjbm(l.gddwbm)) qxj,getzzmc(getGdsbm(l.gddwbm)) gds,getdmbmmc('YHLBDM',j.yhlbdm) yhlb,getdqbm(l.gddwbm) dqbm,${nybm}
                   |from jldxx j
                   |join cbxx c on c.jldbh = j.jldbh and c.dfny = ${nowMonth} and c.sslxdm = '121'
                   |join xlxd l on j.xlxdbs = l.xlxdbs
        ...
        try{
            val xl_18 =
                s"""
                   ...
                   |   getbdzbh(l.xlxdbs) bdzbh,getbdzmc(l.xlxdbs) bdzmc,
                   |   k1.bdzkhbhh,null,y.yhbh,y.yhmc,y.yhlbdm,d.jldbh,b.zcbh,0 fxdl,
                   |   getycgzbh(${Constant.XL_18}) ycgzbh,getDsjbm(l.gddwbm) dsjbm,
                   ...
                   |   getQxjbm(l.gddwbm) qxjbm,getGdsbm(l.gddwbm) gdsbm,getzzmc(getDsjbm(l.gddwbm)) dsj,
                   |   getzzmc(getQxjbm(l.gddwbm)) qxj,getzzmc(getGdsbm(l.gddwbm)) gds,
                   |   getdmbmmc('YHLBDM',y.yhlbdm) yhlb,getdqbm(l.gddwbm) dqbm,${nybm},dc.dydj,
                   ...
                   |    distinct ${creator_id},${create_time},${update_time},${updator_id},
                   |    l.gddwbm,${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,
                   |   l.xlxdbs,l.xlbh,l.xlmc,null tqbs,null tqbh,null tqmc,
                   ...
                   |   getzzmc(getGdsbm(l.gddwbm)) gds,getdmbmmc('YHLBDM',y.yhlbdm) yhlb,
                   |   getdqbm(l.gddwbm) dqbm,${nybm},dc.dydj,getdmbmmc('DYDJDM',dc.dydj) dydj
                   |from ydkh y
                   |join jld d on d.yhbh = y.yhbh
                   |join jlddnbgx g on g.jldbh = d.jldbh
                   |join yxdnb b on b.yxdnbbs = g.yxdnbbs
            ...
            sparkSession.sql("select * from RES_GK_WTJDFDCHDLQD where isFiveDsj(gddwbm) = 1").repartition(resultPartition).createOrReplaceTempView("RES_GK_WTJDFDCHDLQD")
        ...
        try{
            val xl_19 =
                s"""
                   ...
                   |    k2.tqkhbhh,handleNumber(tq.bygdl) gdl,handleNumber(tq.bysdl) sdl,
                   |    handleNumber(tq.byxsl) xsl,0 xgxxs,getycgzbh(${Constant.XL_19}) ycgzbh,
            ...
            sparkSession.sql("select * from RES_GK_XSYCDGBTQQD where isFiveDsj(gddwbm) = 1").repartition(resultPartition).createOrReplaceTempView("RES_GK_XSYCDGBTQQD")

            //线损异常的公变台区清单
            sparkSession.sql(s"select getUUID(),*,tjsj fqrq from res_gk_xsycdgbtqqd")
        ...
        try{
            val xl_110 =
                s"""
                   ...
                   |   j.xlxdbs,xs.xlbh,xs.xlmc,null,null,null,getbdzbs(j.xlxdbs) bdzbs,
                   |   getbdzbh(j.xlxdbs) bdzbh,getbdzmc(j.xlxdbs) bdzmc,
                   |   k1.bdzkhbhh,null,j.yhbh,j.yhmc,j.yhlbdm,handleNumber(b.edrl) byqrl,
                   |   j.jlfsdm,handleNumber(j.ygcjdl+j.ygbsdl) jsdl,
                   |   handleNumber(j.ygcjdl) cjdl,handleNumber(j.ygbsdl) bsdl,
                   |   getycgzbh(${Constant.XL_110}) ycgzbh,getDsjbm(xs.gddwbm) dsjbm,
                   ...
                   |   getdmbmmc('JLFSDM',j.jlfsdm) jlfs,getdqbm(xs.gddwbm) dqbm,${nybm}
                   |from xlxstjxx xs
            ...
            //负线损线路高供低计用户变损电量
            sparkSession.sql(s"select getUUID(),*,tjsj fqrq from res_gk_fsxlbszbdlpdzbkhqd")
...
//            sparkSession.sql(s"insert into ${writeSchema}.GK_FSXLBSZBDLPDZBKHQD_HIS select getUUID(),*,tjsj fqrq from RES_GK_FSXLBSZBDLPDZBKHQD")
        ...
        try{
            ...
            val xl_111 =
                s"""
                   |select
                   |    ${creator_id},${create_time},${update_time},${updator_id},
                   |    l.gddwbm,${nowMonth} tjsj,${ybs} tjzq,${xlbz} xltqbz,
                   |   l.xlxdbs,l.xlbh,l.xlmc,null tqbs,null tqbh,null tqmc,
                   |   getbdzbs(l.xlxdbs) bdzbs,getbdzbh(l.xlxdbs) bdzbh,getbdzmc(l.xlxdbs) bdzmc,
                   |   k1.bdzkhbhh,null,y.yhbh,y.yhmc,y.yhlbdm,j.jldbh,c.zcbh,handleNumber(c.scbss) qm,
                   |   handleNumber(c.bcbss) zm,handleNumber(c.zhbl),
                   |   handleNumber(c.bjdl) dl,f.gzdbh fzqjfgdh,(coalesce(j.jfdl, 0) + coalesce(j.mfdl, 0)) fzqjfdl,
                   |   getycgzbh(${Constant.XL_111}) ycgzbh,
                   |   getDsjbm(l.gddwbm) dsjbm,getQxjbm(l.gddwbm) qxjbm,getGdsbm(l.gddwbm) gdsbm,
                   |   getzzmc(getDsjbm(l.gddwbm)) dsj,
                   |   getzzmc(getQxjbm(l.gddwbm)) qxj,getzzmc(getGdsbm(l.gddwbm)) gds,
                   |   getdmbmmc('YHLBDM',y.yhlbdm) yhlb,getdqbm(l.gddwbm) dqbm,${nybm}
            ...
            sparkSession.sql("select * from RES_GK_FZQXJFDLYHQD where isFiveDsj(gddwbm) = 1").repartition(resultPartition).createOrReplaceTempView("RES_GK_FZQXJFDLYHQD")
